  ...
  return (
    <MagicMenu
      ...
      trigger={
        <IconButton className="relative z-10 " onClick={() => setOpen(!open)}>
          {open ? <XMarkIcon className="size-12" /> : <Bars3Icon className="size-12" />}
        </IconButton>
      }
      fanAngle={180}
      radius={140}
    >
      ...
      <IconButton onClick={() => navigate(Routes.projects)}>
        <CommandLineIcon className="size-12" />
      </IconButton>

      <IconButton className="text-5xl" onClick={() => link(Constants.github, '_blank')}>
      ...
      <IconButton className="text-4xl" onClick={() => link(Constants.linkedIn, '_blank')}>
        <svg width="1em" height="1em" viewBox="0 0 24 24" className="fill-current">
